...
def input_transform_net(point_cloud, is_training, bn_decay=None, K=3):
    ...
    with tf.variable_scope('transform_XYZ') as sc:
        ...
        transform = tf.matmul(net, weights)                                      # (b, 9)
        transform = tf.nn.bias_add(transform, biases)                            # (b, 9)

    transform = tf.reshape(transform, [batch_size, 3, K])                        # (b, 3, 3)
    return transform
...
def feature_transform_net(inputs, is_training, bn_decay=None, K=64):
    ...
    num_point = inputs.get_shape()[1].value                                    # (b, n, 3)

    net = tf_util.conv2d(inputs, 64, [1,1],                                    # (b, n, 1, 64)
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
                         scope='tconv1', bn_decay=bn_decay)
    net = tf_util.conv2d(net, 128, [1,1],                                      # (b, n, 1, 128)
    ...
    with tf.variable_scope('transform_feat') as sc:
        ...
        transform = tf.matmul(net, weights)                                    # (b, 64*64)
        transform = tf.nn.bias_add(transform, biases)                          # (b, 64*64)

    transform = tf.reshape(transform, [batch_size, K, K])                      # (b, 64, 64)
    return transform
